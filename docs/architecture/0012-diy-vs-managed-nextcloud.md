...
- **Pilot result (2026-08):** on the real IONOS instance, **Deck + Collectives install and run** — the app-freedom gate **passed**, contradicting the stale community reports. Open items under test: **Collabora erroring** (browser can't reach the IONOS-provisioned Collabora server — under diagnosis), Talk-thread version, performance, and the T&C/DPA gates. IONOS is now a **legitimate contender**, not a throwaway — though The Good Cloud still leads on values, backups, and clean export.
