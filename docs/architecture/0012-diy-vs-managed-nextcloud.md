...
- **Pilot result (2026-08):** on the real IONOS instance, **Deck + Collectives install and run** — the app-freedom gate **passed**, contradicting the stale community reports. But two managed limitations surfaced: **Talk threads (a required feature) are unavailable** — the instance is on **Talk 21 / Nextcloud 31**, threads need Nextcloud 32, and **managed = no self-upgrade** (IONOS controls the version); and **Collabora works** (the "failed to load" was a Firefox/Privacy Badger cross-domain block, not IONOS — members will need a tracking-protection exception, a DOC-5 note). Still to test: performance, a 10-person call, and the T&C/DPA gates. IONOS is now a **legitimate contender**, but the no-version-control gap and the T&Cs keep **The Good Cloud** ahead on values, backups, currency, and clean export.
