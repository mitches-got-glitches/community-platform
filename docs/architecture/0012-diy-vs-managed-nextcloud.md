...
- **Pilot result (2026-08):** on the real IONOS instance, **Deck + Collectives install and run** — the app-freedom gate **passed**, contradicting the stale community reports. One limitation surfaced and has since resolved: **Talk threads (a required feature) were unavailable** on **Talk 21 / Nextcloud 31** — but **re-checked 2026-09-22, IONOS has moved the instance to Nextcloud 32 / Hub 25 ("Autumn 25"), Talk 22, and threads are enabled**, so the required-feature gap is closed. Note this **confirms** the "~a year behind upstream" reports rather than refuting them — Hub 25 Autumn shipped upstream in **September 2025**. **We accept that lag deliberately:** every required app is now present, and a ~12-month-behind managed instance is a reasonable trade for near-zero ops. The consequence to plan around: **any future required feature is ~12 months out**, and we cannot pull it forward. **Collabora works** (the "failed to load" was a Firefox/Privacy Badger cross-domain block, not IONOS — members will need a tracking-protection exception, a DOC-5 note). Still to test: performance, a 10-person call, and the T&C/DPA gates. **IONOS now passes every required-feature test we have run.** What still separates it from **The Good Cloud** is no longer features but **terms and continuity**: no guaranteed export, no SLA, data-loss liability disclaimed, ad-tech consent at signup, no `occ`, and the structural fact that **we do not control upgrade timing** — a ~12-month lag, accepted, but a **feature-latency risk** rather than a present blocker. Verify the running version is still within upstream **security maintenance**; a year behind is acceptable, out-of-support is not. **Mitigating (confirmed 2026-09-22):** the IONOS contract is **rolling monthly, not a 12-month lock-in** — so leaving is a decision we can take at any month boundary, which lowers the cost of getting this choice wrong. The remaining decision is therefore a **values/continuity judgement, not a capability one**.
