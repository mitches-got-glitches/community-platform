...
**Managed Nextcloud is now the leaning launch path** (lowest bus factor at ≤10 users, near-zero ops), with **provider gated on an app-freedom check** — it must install **Collectives + Deck** and run a working office. Shortlist **The Good Cloud** and **Hetzner Storage Share**. **Update: the IONOS real-data pilot PASSED that gate — Deck + Collectives + Collabora all work** (the office error was a browser/Privacy Badger block; older reports were stale) — and the **Talk-threads gap has since closed** (IONOS moved to NC 32 / Hub 25 / Talk 22 by 2026-09). The "~a year behind upstream" reading is **confirmed, and accepted** — Hub 25 Autumn is a Sept-2025 release — because every required app is now present. So only **T&C/DPA and exit concerns** separate the providers: The Good Cloud leads on values, backups, currency, and clean export, but **no longer on capability**. **Netcup DIY** becomes the **on-trigger** target. **Cost headline:** Netcup restores the budget the Hetzner increase broke (~£270–290/yr all-in for the single-stack plan). Adding a *second* self-hosted stack (Element alongside Nextcloud) pushes past the £300 ceiling — that's a committee budget conversation, not an accident.
...
| **IONOS Managed Nextcloud** | 🇩🇪 DE | ~£130 (£9/mo +VAT; **£1/mo × 3 promo**) | ~£288 (£20/mo) | **NC 32 / Hub 25, Talk 22 — threads ✅** (upgraded by IONOS, re-checked 2026-09-22) | **pilot: Deck + Collectives + Collabora + threads all work** ✅ (office "fail" was a browser/Privacy Badger block); **no `occ`**; **~12 months behind upstream** (Hub 25 Autumn = Sept 2025) — confirmed and accepted |
...
**Pilot update (2026-08):** the real-data pilot **disproves the app-freedom fear** — **Deck, Collectives, and Collabora all work** (the "office failed to load" was a **Firefox/Privacy Badger** cross-domain block, not IONOS; older reports were stale). So IONOS is a **legitimate contender**. The one real IONOS gap — **Talk threads (a required feature)** — **has since closed**: at pilot time the instance ran Talk 21 / Nextcloud 31, but **re-checked 2026-09-22 IONOS had moved it to Nextcloud 32 / Hub 25 ("Autumn 25") with Talk 22 and threads enabled**. This **confirms** the "~a year behind upstream" reports — Hub 25 "Autumn" is a **September 2025** upstream release — but the lag is now **accepted on the record**: every required app is present, and ~12 months behind is a fair price for near-zero ops. The residual is structural rather than functional: **upgrade timing is the provider's call**, so assume **any future required feature is ~12 months out**, and confirm the running version stays within upstream **security maintenance**. Still to test: performance, a 10-person call, and the T&C/DPA gates. Note for rollout: members need a **tracking-protection exception** for office to load (DOC-5). The Good Cloud still leads on values, backups, currency, and clean export — **but no longer on features**.
...
2. **Decide chat direction:** Talk-only for launch (recommended) vs a funded Element spike. → holds or supersedes ADR-0002. **Strengthened 2026-09-22:** with Talk 22 threads live on the managed instance, Talk meets the full chat requirement (channels, membership, replies, threads) with no extra stack — an Element spike is now preference, not necessity. Caveat: on managed we sit **~12 months behind upstream**, so future Talk improvements arrive on the provider's schedule.
